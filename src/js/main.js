...
;(function( $ ){
    ...
    var DanmuPlayer= function (element, options) {
        ...
        this.$ctrl.append('<div class="ctrl-progress"><div class="current">' +
            '<div class="progress-handle"><span class="  glyphicon glyphicon-expand" aria-hidden="true"></span></div></div>' +
        ...
        $(this.id+ " .ctrl-progress").on('click',{video:this.video,that:that},function(e) {
            ...
            //console.log(e.data.that.duration);
        ...
        var timeDrag = false;
        $(this.id+ " .ctrl-progress").on('mousedown',function(e) {
            timeDrag = true;
        });
        $(document).on('mouseup',function(e) {
            if(timeDrag) timeDrag = false;
        });
        $(this.id+ " .ctrl-progress").on('mousemove',{video:this.video,that:that},function(e) {
            if(timeDrag) {
                var sumLen=$(e.data.that.id+" .ctrl-progress").width();
                var pos= e.pageX-$(e.data.that.id+" .ctrl-progress").offset().left;
                var percentage=pos/sumLen;
        ...
        //用户操作控制条事件
        $(this.id+" .ctrl-progress").on("mouseup",{that:that},function(e){
